...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if($data['is_dealer']){
			$data['current_currency'] = (isset($_SESSION['current_currency'])) ? $_SESSION['current_currency'] : CURRENCY_BASIC;
		}
		...
		if ($product_info) {
			...
			$data['text_smartphone'] = $this->language->get('text_smartphone');
			...
			$data['product_produtct_shipping_top_title'] = $this->language->get('product_produtct_shipping_top_title');
			$data['product_produtct_shipping'] = $this->language->get('product_produtct_shipping');
			$data['product_produtct_shipping_bottom_title'] = $this->language->get('product_produtct_shipping_bottom_title');
			$data['text_favorites'] = $this->language->get('text_favorites');
			$data['text_characteristics'] = $this->language->get('text_characteristics');
			$data['text_characteristics_major'] = $this->language->get('text_characteristics_major');
			$data['text_characteristics_all'] = $this->language->get('text_characteristics_all');
			$data['text_product_in_stock'] = $this->language->get('text_product_in_stock');
			$data['text_product_nope_stock'] = $this->language->get('text_product_nope_stock');
			...
			if($data['is_dealer']){
				$priceData = $this->currency->getProductCurrencyData(
					$product_info['price_dealer'], $this->customer->config->get('config_customer_group_id'), $data['current_currency']);
			}else{
				$priceData = $this->currency->getProductCurrencyData($product_info['price']);
			}
			
			$product_info['price'] = $priceData['price'];
			$product_info['only_price'] = $priceData['only_price'];
			$product_info['currency_right'] = $priceData['currency_right'];
			$product_info['only_special'] = number_format($product_info['special'], 0, '.', '');
			
			$data['product_info'] = $product_info;
			
				//	Favorites
			$data['all_favorites'] = -1;
			$data['favorites_vk'] = 11;
			$data['favorites_fb'] = 0;
			$data['favorites_tw'] = 3;
			$data['favorites_gplus'] = 0;
			$data['favorites_other'] = 0;
			...
			$data['reviews'] = sprintf($this->language->get('text_reviews_pattern'), (int)$product_info['reviews']);
			...
			$data['products'] = array();
				...
				//	Set path GET paran for filter module
			$categoryData =  $this->model_catalog_product->getCategoryData(
				$this->request->get['product_id'], 
				$this->config->get('config_language_id'), 
				array('category_id', 'name')
			);
			$this->request->setGet('path', $categoryData[0]['category_id']);
			$data['link_catalog_url'] = $this->url->link('product/category&path='. $categoryData[0]['category_id']);
			...
			foreach ($results as $result) {
				...
				if($data['is_dealer']){
					...
					$priceData = $this->currency->getProductCurrencyData($result['price_dealer']);
				}
				...
				$data['products'][] = array(
					...
					'description' => utf8_substr(strip_tags(trim(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8'))), 0, 110),// $this->config->get('config_product_description_length') . '..',
					...
					'href'        => $this->url->link('product/product', 'product_id=' . $result['product_id'] . $url)//$this->url->link('product/product', 'path=' . $this->request->get['path'] . '&product_id=' . $result['product_id'] . $url)
				);
			}
